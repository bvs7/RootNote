...
class MC_State(FactionState):
  ...
  def get_crafting_ability(self):
    ...
    for c in self.board_state.clearings:
      n_ws = len(ws for ws in c.buildings if ws == Building.workshop)
  ...
  def recruit(self, cs:List[int]):
    rec_track = self.tracks[Building.recruiter]
    ...
    if n_rec > self.warrior_supply:
      ...
      for c in self.board_state.clearings:
        n_r = len([rec for rec in c.buildings if rec == Building.recruiter])
